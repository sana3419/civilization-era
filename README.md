...
**铁锈战争式选中上下文：一切功能挂在选中上，无常驻按钮**

- 左键拖动 = 平移地图；左键点选：单位优先，其次建筑；Esc 取消；Ctrl+A 全选军队，Ctrl+W 全选工人
- 右键拖动 = 框选；右键点击 = 命令（敌人集火/石墙登墙/修理/采集树石/移动）；S = 停止
- **选中工人** → 底部出现建造菜单（经济/军防两行）+ 停止按钮
- **选中军事单位** → 命令行 + 阵型 8 钮（或 F1~F8）；多类型混选出现类型 chips（左键点 chip 只选该类、右键剔除）
- **点选建筑** → 名称/HP 面板；兵营/射箭场/马厩/攻城工坊出现**训练按钮**，选中后右键地面 = 设集结点（新兵自动前往）；城门面板有开/关按钮
- Ctrl+1~9 设编队，1~9 选编队；WASD 平移相机，滚轮缩放；小地图点击/拖动跳转
- 建造放置：幽灵预览（绿可放/红不可）→ 左键放置**工地**（半透明，扣资源），选中的工人自动前往施工，
  补满血即完工生效（各建筑施工时长不同，多工人可叠加；盖完自动接附近下一个工地）；Shift 连放，右键/Esc 取消
- 工地无功能：箭塔不射击、农田不产粮、不收资源、不能训练、门不可通行；可被土匪攻击
- 栅栏/石墙按住左键拖动划线连放（开门只放行己方，土匪会攻门）
