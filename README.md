...
- 市场可把过剩木材换石/食（选中市场出兑换栏，比采集贵、应急用）
- 锯木厂自动把木材加工成木板（5→3/4s），攻城槌/投石车需要木板
- 单位要吃饭：≈1 食物/分/人自动消耗，断粮采集/修理减半、士气基线下滑——农田常备
...
- Space 暂停；Ctrl+A 全选军队 / Ctrl+W 全选工人；S 停止（清命令队列并原地停）
- Shift+右键 = 命令排队（待命后依次执行）；训练 4 秒/个有队列与进度条
- 左键空点己方建筑 = 选中（状态行显示血量/训练队列），再右键 = 设集结点（新兵出厂自动前往）
