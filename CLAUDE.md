...
第三阶段（经济深度，按评审压缩）批次1已落地：**食物消耗环**——
全员饱食度（satiety，SAVE_VERSION v9），≈1 食物/分/人自动进食，
断粮 → 采集/修理 ×0.5 + 士气基线压到 40；HUD 显示消耗速率与饥饿预警。
批次2已落地：**两层加工链**——新资源木板（RES_PLANK，SAVE_VERSION v10）、
锯木厂自动 5木→3板/4s（串行按建筑序）、攻城槌/投石车改吃木板；
try_spend/debug_add_resources 第 4 参默认 0（旧调用兼容）。
批次3已落地：**市场**（25木5石）——选中后底栏出兑换按钮
（10木→3石 / 10木→5食 / 5板→4石），sim 侧 trade() 只做原子交换，
汇率在 main.gd 的 TRADES 表。第三阶段剩余（按评审压缩）：
村公所/政策点（等政策系统一起）、商队贸易（等多聚落）——暂缓。
